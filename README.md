...
<!-- 素材位：Media/gameplay.gif，建议展示“换位 → 连锁触发 → 牌型倍率 → 通关” -->
...
<!-- 素材位：Media/catalog.png，展示卡牌类型与内容规模 -->
...
<!-- 素材位：Media/config-table.png，展示配置表局部 -->

<!-- 素材位：Media/import-tool.png，展示 Unity 导入工具与生成后的 SO -->

### 存档

游戏状态通过 JSON 持久化，并采用阶段存档：在进关、离店和关卡结算等稳定边界统一写盘。这样既能避免保存到流程中间态，也为玩家提供了清晰的 S/L（Save/Load）节点。

<!-- 素材位：Media/archive-json.png，展示格式化后的 JSON 存档局部 -->
